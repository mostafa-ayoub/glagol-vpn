// ============================================================
// services/fixed_vpn_service.dart
// ============================================================
//
// Fixed VPN service that launches external VPN apps
...
class FixedVpnService {
  static final FixedVpnService _instance = FixedVpnService._internal();
  factory FixedVpnService() => _instance;
  FixedVpnService._internal();
  ...
  Future<bool> connect(VpnServer server, {String? vpnAppName}) async {
    ...
    try {
      ...
      debugPrint('[FixedVpnService] Connection error: $e');
  ...
  Future<bool> _launchVPNApp() async {
    try {
      ...
      debugPrint('[FixedVpnService] Launch error: $e');
      return false;
    }
  }
  ...
  Future<void> _checkVPNStatus() async {
    try {
      final ip = await _getCurrentIP();
      final isConnected = await _isVPNActive(ip);
      ...
      if (isConnected && status.value != VpnStatus.connected) {
        ...
        _startStatsTimer();
        currentIP.value = ip;
        debugPrint('[FixedVpnService] VPN Connected! IP: $ip');
        ...
        debugPrint('[FixedVpnService] VPN Disconnected');
      } else if (isConnected) {
        // Update IP if it changed
        currentIP.value = ip;
      }
    } catch (e) {
      debugPrint('[FixedVpnService] Status check error: $e');
    }
  }

  // ── Check if VPN is Active ───────────────────────────────
  Future<bool> _isVPNActive(String? ip) async {
    if (ip == null || _originalIP == null) return false;

    // Check if IP changed (strong indicator of VPN)
    if (ip != _originalIP) return true;
    ...
    try {
      ...
      debugPrint('[FixedVpnService] Interface check error: $e');
    }
  ...
  Future<String?> _getCurrentIP() async {
    try {
      ...
      debugPrint('[FixedVpnService] IP check error: $e');
    }
    return null;
  }
